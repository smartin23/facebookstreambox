...
class plgContentfacebookstreambox extends JPlugin 
{
	...
	function onContentPrepare( $context, &$row, &$params, $limitstart=0 )
	{
		...
		$apiResult = $facebook->api("/".$fan_id, "get");		
		$pageLink = $apiResult['link'];
			
		$url="<div class=\"my_facebook_link\"><a href=\"".$apiResult['link']."\" target=\"_blank\"><img style= \"float:left; margin-right:15px;\" src=\"https://graph.facebook.com/".$fan_id."/picture?height=80\"><div class=\"my_facebook_link_title\">Suivez-nous sur Facebook</div><div class=\"my_facebook_link_name\">".$apiResult['name']."</div></a></div>";
		
		$fql    =   "SELECT post_id, actor_id, target_id , created_time, message, likes, attachment FROM stream WHERE source_id = '".$fan_id."'  ORDER BY created_time DESC LIMIT 20";
		$param  =   array(
